...
def bagging(dataSet,labels,train_number = 5):
...
def baggingPredict(X, trees,labels):
    result=[]
    for tree in trees:
...
def pltAdaBoostDecisionBound(X_, y_, trees,labels):
    ...
    Z_ = baggingPredict(np.c_[X_tmp.ravel(), Y_tmp.ravel()], trees,labels).reshape(X_tmp.shape)
...
def main():
    ...
    trees = bagging(dataSet,labelSet)
    X = dataSet[:,:-1]
    y = dataSet[:,-1]
    y[y == 0] = -1
