...
class KMeans(object):
    ...
    def fit(self, X, initial_centroid_index=None, max_iters=10, seed=16, plt_process=False):
        ...
        for i in range(max_iters):
            ...
            if plt_process:
                self.plot_converge(X, idx, centroid)

            # 重新计算中心点
            newcentroid = self.compute_centroids(X, idx)
            # 不在变化时停止
    ...
    def plot_converge(self, X, idx, centerpoint):
        plt.cla()  # 清除原有图像
        ...
        # 标记初始化中心点
        plt.scatter(centerpoint[:, 0], centerpoint[:, 1], label='initial center', c='k')
...
if __name__ == '__main__':
    ...
    data = np.loadtxt('../DataSet/watermelon4_0_Ch.txt', delimiter=', ')
