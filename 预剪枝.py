...
def createTree(dataSet, labels,data_test):
    ...
    if bestFeatLabel in labels:
        ...
        for value in uniqueVals:
            ...
            data_testchridren=splitDataSet(data_test,bestFeatLabel,value)
            if(len(dataSetchridren)==0):
                return majorityCnt(dataSet,labelscopy)
            # 递归调用创建决策树
            myTree[bestFeatLabel][value] = createTree(dataSetchridren, labelscopy,data_testchridren)
    #对连续值的处理
    else:
        ...
        left_test,right_test=splitContinuousDataSet(data_test,bestFeat,bestPartValue)
        # 构建左子树
        valueLeft = 'Y'
        myTree[bestFeatLabel][valueLeft] = createTree(left,subLabels,left_test)
        # 构建右子树
        valueRight = 'N'
        myTree[bestFeatLabel][valueRight] = createTree(right,subLabels,right_test)

    # 进行测试，若划分没有提高准确率，则不进行划分，返回该节点的投票值作为节点类别
    if testing(myTree, data_test, temp_labels) < testingMajor(majorityCnt(dataSet,labels), data_test,temp_labels):
        return myTree
    return majorityCnt(dataSet,labels)
...
def main():
    ...
    # data_test=dataSet[:3]+dataSet[13:]
